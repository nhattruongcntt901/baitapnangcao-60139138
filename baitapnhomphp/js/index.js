...
function ds_nhac() {
    ...
    xhttp.open('GET', 'chucnang/list_song.php', true);
    xhttp.send();
}
ds_nhac_playlist();
...
async function ds_nhac_sort() {
    de_muc.classList.add("animate__rotateOutUpLeft");
        await sleep(400);
         de_muc.classList.remove("animate__rotateOutUpLeft");
         de_muc.classList.add("animate__rotateInUpLeft");
        de_muc.innerHTML = "XẾP HẠNG BÀI HÁT";
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            document.getElementById('list_song').innerHTML =
    ...
    xhttp.open('GET', 'chucnang/list_song.php?sapxep=true', true);
    xhttp.send();
}
...
function add_to_playlist(id){
    ...
    var xhttp = new XMLHttpRequest();
    xhttp.open('GET', 'chucnang/add_to_playlist.php?id_nhac=' + id_nhac , true);
    xhttp.send();
}
